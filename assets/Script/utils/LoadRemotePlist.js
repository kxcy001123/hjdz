...
module.exports = function (plisturl, atlasUrl ,callback) {
    cc.assetManager.loadRemote(plisturl, function (err, plist) {
        if (err) {
            return callback(err);
        }

        let _plist =  cc.plistParser.parse(plist.text)

        let texture = _plist.metadata.realTextureFileName || _plist.metadata.textureFileName;


        texture = atlasUrl ? atlasUrl :cc.path.join(cc.path.dirname(plisturl), texture);
        cc.assetManager.loadRemote(texture, function (err, tex) {
            if (err) {
                return callback(err);
            }
            let atlasSprite = parsePlist(_plist, tex);
